function [ X1, Z1 ] = Optimal_region_surface_single(varX, X, res, NumberofHiddenNeurons, par, OutputWeight, ActivationFunction )
...
%     for ii=1:res % here the conversion from normalised to real data again is based on the fact you normalised between 0-1
        X1(jj)= min(raw_data4training(:,varX)) + jj*((max(raw_data4training(:,varX)) - min(raw_data4training(:,varX)))/res);
%     end
end
% create Y vector
% for jj=1:res
